    ...
    <div class="page-content d-flex align-items-stretch">
        ...
        <div class="content-inner">
            ...
            <header class="page-header">
                <div class="container-fluid">
                    <h2 class="no-margin-bottom">Panel Usuario</h2>
                </div>
            </header>
                        ...
                        <div class="col-lg-12" id="reporte" style="display: none">
                            <div class="card">
                                ...
                                <div class="card-header d-flex align-items-center">
                                    ...
                                    <hr>
                                    <button class="btn btn-sm btn-info" onclick="$('#reporteModal').modal();cagarGrafica();">Reporte Grafico</button>
                                </div>
                            ...
                            <div class="modal fade" id="reporteModal" role="dialog">
                                <div class="modal-dialog modal-lg">
                                    <div class="modal-content">
                                        <div class="modal-header">
                                            <h4 class="modal-title">Cantidad parqueo vs Dias</h4>
...
<script>
    ...
    function cagarGrafica() {
            post(url+"/gestionParking/3",{},"POST",csrf_token).then(data=>{
                var fec = Object([]);

                fec.l = 0,fec.m = 0,fec.mi=0,fec.j=0,fec.v = 0,fec.s=0,fec.d=0;

                for(var i in data){
                var dia = getday(data[i].created_at);
                switch (dia) {
                    case "Lunes": fec.l++;break;
                    case "Martes": fec.m++;break;
                    case "Miercoles": fec.mi++;break;
                    case "Jueves": fec.j++;break;
                    case "Viernes": fec.v++;break;
                    case "Sabado": fec.s++;break;
                    case "Domingo": fec.d++;break;
                    }
                }
                var fec = [fec.l,fec.m,fec.mi,fec.j,fec.v,fec.s,fec.d];
    ...
    function loaderReporte(imprimir = 0){
        post(url+"/gestionParking/3",{},"POST",csrf_token).then(data=>{
            var string = "";
            for(var i in data){
                string += "<tr>";
    ...
    function  imprimirTicket(data) {
        ...
        var fechaEntrada = data.created_at;
        var estado =  (data.estado == 1) ? 'Parqueado' : 'no Parqueado';
        var fechaSalidad =data.updated_at;


        var data = {cedula,placa,puesto,fechaEntrada,fechaSalidad,estado};

        var data  = new Array(data);
        var columns = [
            {title: "Cedula", dataKey: "cedula"},
            {title: "Placa del vehiculo", dataKey: "placa"},
            {title: "Puesto hubicado", dataKey: "puesto"},
            {title: "Fecha Entrada", dataKey: "fechaEntrada"},
